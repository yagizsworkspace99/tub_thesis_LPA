...
private:
    ...
    void batchOperationCuckooParlay(bool flag,libcuckoo::cuckoohash_map<uint64_t, libcuckoo::cuckoohash_map<uint64_t, std::vector<uint64_t>>>& groupedData, uint64_t maxTime, std::unordered_map<uint64_t, uint64_t> uniqueTimesMap);
    ...
    void memoryConsumption (libcuckoo::cuckoohash_map<uint64_t,libcuckoo::cuckoohash_map<uint64_t, std::vector<uint64_t>>> &map);
};
